				...
				if (options.navigation == 1){ //Skip if no navigation
					...
					$('#nextslide img').hover(function() {
						if($.paused == true) return false;
					   	$(this).attr("src", "/Announcement-Rotator/Images/forward.gif");
					}, function(){
						if($.paused == true) return false;
					    $(this).attr("src", "/Announcement-Rotator/Images/forward_dull.gif");
					});
					$('#prevslide img').hover(function() {
						if($.paused == true) return false; 
					    $(this).attr("src", "/Announcement-Rotator/Images/back.gif");
					}, function(){
						if($.paused == true) return false;
					    $(this).attr("src", "/Announcement-Rotator/Images/back_dull.gif");
					});
					
				    //Play/Pause Button
				    $('#pauseplay').click(function() {
				    	if($.inAnimation) return false;
				    	var src = ($(this).find('img').attr("src") === "/Announcement-Rotator/Images/play.gif") ? "/Announcement-Rotator/Images/pause.gif" : "/Announcement-Rotator/Images/play.gif";
      					if (src == "/Announcement-Rotator/Images/pause.gif"){
      						$(this).find('img').attr("src", "/Announcement-Rotator/Images/play.gif");
      						$.paused = false;
					        slideshow_interval = setInterval(nextslide, options.slide_interval);  
				        }else{
				        	$(this).find('img').attr("src", "/Announcement-Rotator/Images/pause.gif");
				        	clearInterval(slideshow_interval);
				        	$.paused = true;
				        }
				    ...
				    $('#pauseplay').mouseover(function() {
				    	var imagecheck = ($(this).find('img').attr("src") === "/Announcement-Rotator/Images/play_dull.gif");
				    	if (imagecheck){
      						$(this).find('img').attr("src", "/Announcement-Rotator/Images/play.gif"); 
				        }else{
				        	$(this).find('img').attr("src", "/Announcement-Rotator/Images/pause.gif");
				    ...
				    $('#pauseplay').mouseout(function() {
				    	var imagecheck = ($(this).find('img').attr("src") === "/Announcement-Rotator/Images/play.gif");
				    	if (imagecheck){
      						$(this).find('img').attr("src", "/Announcement-Rotator/Images/play_dull.gif"); 
				        }else{
				        	$(this).find('img').attr("src", "/Announcement-Rotator/Images/pause_dull.gif");
				        }
				        return false;
				    });
				}
		...
		$('#supersize > *').hover(function() {
	   		if (options.slideshow == 1 && options.pause_hover == 1){
	   			if(!($.paused) && options.navigation == 1){
	   				$('#pauseplay > img').attr("src", "/Announcement-Rotator/Images/pause.gif"); 
	   				clearInterval(slideshow_interval);
	   			}
	   		}
			...
			if (options.slideshow == 1 && options.pause_hover == 1){
				if(!($.paused) && options.navigation == 1){
					$('#pauseplay > img').attr("src", "/Announcement-Rotator/Images/pause_dull.gif");
					slideshow_interval = setInterval(nextslide, options.slide_interval);
				} 
			}
	...
	$.fn.resizenow = function() {
		var options = $.extend($.fn.supersized.defaults, $.fn.supersized.options);
	  	return this.each(function() {
			...
			var browserwidth = 615; //$(window).width()
			var browserheight = 350; //$(window).height();
			var offset;
